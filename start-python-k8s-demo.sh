...
echo "    thread-leaky-api-python   — thread count grows (OOM ~15 min)"
echo "    gc-pressure-api-python    — GC gen2 collections grow (OOM ~15 min)"
echo "    native-leaky-api-python   — native malloc leak, RSS >> heap (OOM ~15 min)"
echo ""
...
echo "    - Use a time range that starts after the last pod restart"
